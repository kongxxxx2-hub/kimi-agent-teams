...
LEADER_REVIEW_PROMPT = """你是审查专家。审核前必须先用搜索工具核实关键数据，不要凭记忆质疑。

## 最重要的规则
- 质疑任何数据之前，必须先搜索验证。如果搜索结果证明报告数据是对的，不要打回
- 不要把正确的数据改错。宁可放过一个小问题，也不要引入新的错误

## 审核维度

### 1. 数据准确性（必须搜索验证后再判断）
- 关键数字（股价、市值、营收、市占率）是否与最新公开数据一致？
- 搜索验证后发现数据确实有误才标记为问题
...
## 判断标准
- 以上 5 项都没有重大问题 → pass
- 有实质性缺失（某个产业链环节完全没提、关键数据确实错误）→ revise
    ...
    def _leader_review(self, task_id, plan, combined_output):
        ...
        for review_round in range(1, MAX_REVIEW_ROUNDS + 1):
            ...
            revision_input = (
                ...
                f"你之前的产出:\n{combined_output}\n\n"
                f"修订要求：\n"
                f"1. 对 Leader 指出的每个问题，必须先用搜索工具核实，不要盲目接受\n"
                f"2. 如果搜索证明你原来的数据是对的，保留原数据并说明\n"
                f"3. 如果搜索证明确实有误，用搜索到的最新数据更正\n"
                f"4. 补充 Leader 指出的缺失内容\n"
                f"5. 输出完整的修订后报告（不是只输出修改部分）"
            )
